...
class ControllerExtensionModuleImagesfinder extends Controller {
	...
	public function index() {
		...
		$data = $this->load->language('extension/module/images_finder');

		$this->document->addStyle('view/stylesheet/opencartmodule/main.css');
		$this->document->addStyle('view/stylesheet/opencartmodule/pe-icon-7-stroke.min.css');
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/images_finder', 'token=' . $this->session->data['token'], true)
		);

		$data['action'] = $this->url->link('extension/module/images_finder', 'token=' . $this->session->data['token'], true);
